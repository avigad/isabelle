...
theorem (in prob_space) central_limit_theorem_zero_mean:
  ...
  shows "weak_conv_m
    (\<lambda>n. distr M borel (\<lambda>x. S n x / sqrt (n * \<sigma>\<^sup>2)))
    std_normal_distribution"
proof -
  let ?S' = "\<lambda>n x. S n x / sqrt (real n * \<sigma>\<^sup>2)"
    and ?m = "\<lambda>x. min (6 * x\<^sup>2)"
  ...
  proof (rule eventually_sequentiallyI)
    ...
    have "norm (\<psi> n t - (1 - ?t^2 * \<sigma>\<^sup>2 / 2)) \<le>
        ?t\<^sup>2 / 6 * ?I n t"
    ...
    have "norm (\<phi> n t - (of_real (1 + (-(t^2) / 2) / n))^n) \<le>
        n * norm (\<psi> n t - (of_real (1 + (-(t^2) / 2) / n)))"
  ...
  proof (rule levy_continuity)
    fix t
    have "(\<lambda>n. ?m x (\<bar>t\<bar> * \<bar>x\<bar> ^ 3 / \<bar>sqrt (\<sigma>\<^sup>2 * real n)\<bar>))
    ...
    have "(\<lambda>n. complex_of_real ((1 + (-(t^2) / 2) / n)^n)) \<longlonglongrightarrow>
      ...
      by (rule isCont_tendsto_compose [OF _ tendsto_exp_limit_sequentially])
         auto
    then have "(\<lambda>n. \<phi> n t) \<longlonglongrightarrow> of_real (exp (-(t^2) / 2))"
...
theorem (in prob_space) central_limit_theorem:
  ...
  shows "weak_conv_m
    (\<lambda>n. distr M borel (\<lambda>x. (S n x - n * c) / sqrt (n*\<sigma>\<^sup>2)))
    std_normal_distribution"
proof -
  have "weak_conv_m
  ...
  proof (intro central_limit_theorem_zero_mean)
    ...
    show "distr M borel (\<lambda>x. X n x - c) = 
        distr \<mu> borel (\<lambda>x. x - c)" for n
...
theorem Skorohod:
  ...
  shows
    "\<exists>(\<Omega> :: real measure)
      (Y_seq :: nat \<Rightarrow> real \<Rightarrow> real) (Y :: real \<Rightarrow> real).
...
lemma (in real_distribution) cdf_lim_at_top_prob:
  "(cdf M \<longlongrightarrow> 1) at_top"
text_raw \<open>}%EndSnippet\<close>
...
lemma dominated_convergence:
  ...
  assumes "f \<in> M \<rightarrow>\<^sub>M borel" "\<And>i. s i \<in> M \<rightarrow>\<^sub>M borel"
     "integrable M w"
...
lemma integrable_iff_bounded:
  ...
  shows "integrable M f \<longleftrightarrow>
    f \<in> M \<rightarrow>\<^sub>M borel \<and> (\<integral>\<^sup>+x. norm (f x) \<partial>M) < \<infinity>"
text_raw \<open>}%EndSnippet\<close>
...
lemma integral_norm_bound:
  fixes f :: "_ \<Rightarrow> 'a :: {banach, second_countable_topology}"
  shows "integrable M f \<Longrightarrow>
    norm (LINT x|M. f x) \<le> (LINT x|M. norm (f x))"
text_raw \<open>}%EndSnippet\<close>
...
lemma interval_integral_FTC2:
  ...
  assumes "a \<le> c" "c \<le> b" "continuous_on {a..b} f"
    "a \<le> x" "x \<le> b"
...
lemma interval_integral_substitution_integrable:
  ...
  assumes "a < b"
    ...
    and "set_integrable lborel
     (einterval a b) (\<lambda>x. g' x *\<^sub>R f (g x))"
    and "set_integrable lborel
     (einterval A B) (\<lambda>x. f x)"
...
lemma (in real_distribution) char_zero:
  "char M 0 = 1"
text_raw \<open>}%EndSnippet\<close>
  by (rule char_zero)
...
lemma (in prob_space) char_approx3':
  ...
  assumes "random_variable borel X"
    and "integrable M X" "integrable M (\<lambda>x. (X x)^2)"
    and "expectation X = 0"
...
lemma std_normal_moment_even:
  "has_bochner_integral lborel
    (\<lambda>x. std_normal_density x * x ^ (2 * k))
    (fact (2 * k) / (2^k * fact k))"
text_raw \<open>}%EndSnippet\<close>
...
lemma std_normal_moment_odd:
  "has_bochner_integral lborel
    (\<lambda>x. std_normal_density x * x^(2 * k + 1)) 0"
text_raw \<open>}%EndSnippet\<close>
...
theorem Helly_selection:
  ...
  shows "\<exists>s. subseq s \<and>
    (\<exists>F. (\<forall>x. continuous (at_right x) F) \<and>
      mono F \<and> (\<forall>x. \<bar>F x\<bar> \<le> M) \<and>
...
theorem tight_imp_convergent_subsubsequence:
  ...
  shows "\<exists>r M. subseq r \<and> real_distribution M \<and>
    weak_conv_m (\<mu> \<circ> s \<circ> r) M"
text_raw \<open>}%EndSnippet\<close>
...
where
  "density M f =
    measure_of (space M) (sets M) 
      (\<lambda>A. \<integral>\<^sup>+ x. f x * indicator A x \<partial>M)"
text_raw \<open>}%EndSnippet\<close>
end
...
where
  "indep_sets F I \<longleftrightarrow>
    ...
    (\<forall>J\<subseteq>I. J \<noteq> {} \<longrightarrow> finite J \<longrightarrow> (\<forall>A\<in>(\<Pi> i\<in>J. F i).
         prob (\<Inter>j\<in>J. A j) = (\<Prod>j\<in>J. prob (A j))))"
text_raw \<open>}%EndSnippet\<close>

text_raw \<open>\DefineSnippet{indepevents}{\<close>
definition (in prob_space)
  "indep_events A I \<longleftrightarrow> indep_sets (\<lambda>i. {A i}) I"
text_raw \<open>}%EndSnippet\<close>

text_raw \<open>\DefineSnippet{indepvars}{\<close>
definition (in prob_space)
  indep_vars ::
    "('i \<Rightarrow> 'b measure) \<Rightarrow> ('i \<Rightarrow> 'a \<Rightarrow> 'b) \<Rightarrow> 'i set \<Rightarrow> bool"
where
...
where
  "normal_density \<mu> \<sigma> x =
     1 / sqrt (2 * pi * \<sigma>\<^sup>2) * exp (-(x - \<mu>)\<^sup>2/ (2 * \<sigma>\<^sup>2))"
