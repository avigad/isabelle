...
% Attempt to fix broken flow. (Luke)
...
% 'nondecreasing in x' sounds strange for me. The variable x is specified in the expression F_\mu(x); is there any reason to say nondecreasing in x? (Luke)
...
% Changed wording from 'approach mu weakly' to 'converge weakly to mu,' in accordance with standard nomenclature. Is there a reason for the alternate
% wording? (Luke)
\begin{definition}
...
Here is our formulation in Isabelle [TODO: change $S$ below to $S'$?]: % Why? (Luke)
...
% Removed 'essentially' from 'essentially a Fourier transform,' and quotes, because the characteristic function literally is a Fourier transform.
% Could say 'the Fourier transform,' but uniqueness depends on the definition of Fourier transform (some people divide by 2pi, etc.)
