...
In Billingsley~\cite{billingsley:95} many proofs are performed using $\varepsilon$-$\delta$-proofs (e.g. the Helly selection theorem). This works well when the domain is the set of real numbers, or is at least a metric space. It turns out that in this specific case the domain is $\ennRR$, for two reasons: (1) we reason about $\sigma$-finite measures, i.e. there are measurable sets for which the measure is infinite, (2) we reason about $\liminf$ and $\limsup$, which are defined on complete lattices but not on the real numbers. For the proof of the Helly selection theorem it turns out that an order-theoretic approach is more appropriate. We still have limits and filters, but instead of obtaining a $\varepsilon$-neighborhood of a specific point, we get a lower bound for which the property still holds. Working with a positive $\varepsilon$ where one needs to compute the difference is difficult, because subtraction is not well behaved in $\ennRR$ (nor is it well behaved on the extended real numbers).

\[ F~x = \bigsqcup_{x < r~n} \lim_{k \to \infty} f~(d~k)~(r~n) \]
\[ \lim_{n \to \infty} f~(d~n)~x = F~x \quad\textrm{if $F$ is continuous at $x$} \]

%
% Old proof (derived from Billingsley):
%
% - we get a \varepsilon > 0, hence F x + e > F x = \bigsqcup ..., hence a n exists 
%   this is fine, but we could also work with u > F x!
% 
% - 
%
%
...
Also many textbook proofs apply a $\varepsilon$-$\delta$-method where they obtain multiple $\varepsilon$ by destructing elements, i.e.~diameters of neighbourhoods in open sets or neighbourhoods in the range of continuous functions.  Finally, the properties and all $\varepsilon$ are combined and a final $\delta$-value needs to be computed and proved correct. It turns out that many such proofs can be simplifed by keeping the relation with certain filter, i.e.~that multiple properties hold in a neighbourhood now expressed by the filter, then the filter can be transformed by various operations (e.g.~by applying a continuous functions) and finally we derive the end result. This safes us from explicitly carrying around the $\varepsilon$ values as well as constructing a final $\delta$ value.

%
% Example in Levy's uniqueness theorem: 
%
% - simplified variant talks about |cdf M1 x - cdf M2 x| instead of cdf M1 and cdf M2 separately
%   i.e. we keep the continuity information. we only need to show that it holds for two e/2 and not four e/4 
%
% - for at_right x and at_bot we get the information together with x as bound.
%
