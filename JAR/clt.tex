...
To provide a good support for automation, Isabelle's multivariate analysis provides a big set of introduction rule to prove openness or closedness of sets, or continuity of functions. Especially for continuity it is important to state the rules in the right format. Following the usual mathematic conventions, the statements that, e.g.~multiplication and $\ln$ are continuous, would be written like $\mathtt{continuous\_on}~(\RR \times \RR)~(\lambda (x, y).~x * y)$ and $\mathtt{continuous\_on}~(0, \infty]~\ln$. Now, this rules would not fit to prove continuity of $\lambda x.~\ln (1 + x * x)$: we would need to explicitly apply composition of continuous functions, and continuity of pair construction. Hence the continuity rules are stated as follows:
\[ \mathtt{continuous\_on}~A~f \land (\forall x \in A.~0 < f~x) \implies
  \mathtt{continuous\_on}~A~(\lambda x.~\ln~(f~x)) \]
\[ \mathtt{continuous\_on}~A~f \land \mathtt{continuous\_on}~A~g \implies
  \mathtt{continuous\_on}~A~(\lambda x.~f~x + g~x)) \]
Now, the conclusions are parametric in the range (i.e.~$A$) and any further function (i.e.~$f$ and $g$). To prove $\lambda x.~\ln (1 + x * x)$, we just apply a fixed set of introduction rules about continuity. Then we end up with the goal $\forall x.~0 < 1 + x * x$, which is proved by the simplifier. This idea goes back to Gottliebsen~\cite{gottliebsen:00}, where it is implemented in PVS.
...
Another way to define a measure $\nu$ on a space $\mdl M$ is to take a nonnegative measurable function $f$ from $\mdl M$ to the extended non-negative real numbers, and define, for every set $A$, $\nu(A) = \int_A^+ f \; d\mdl M$. This is defined formally in the Isabelle library as follows:
\Snippet{density}
The integral $\int_A^+ f \; d\mdl M$ is the nonnegative Lebesgue integral, defined for functions into the extended non-negative numbers. For measurable functions it has the expected properties: closed under addition, constant multiplication and monotone convergence. It is monotone even for non-measurable functions, which simplifies certain proofs as measurability is not always easy to prove.
...
The Bochner integral of $f$ is defined using a sequence of simple functions $s_i$ converging towards $f$ in norm. The distance of two functions is defined as $|| s_i - f || = \int_x^+ |s_i~x - f~x| d\mdl M$, using the Lebesgue integal. Simple functions are measurable functions with a finite range and finitely measurable support. Hence the integral of a simple function is defined as a sum over the range. The Bochner integral is then the limit of the integrals of $s_i$. A function $f$ is Bochner integrable when $f$ is measurable and the norm of $f$ is finite, i.e.~it is equivalent to absolutely integrable on the Lebesgue measure:
...
The fundamentals of measure theory are found in the \texttt{HOL-Probability} library, which includes \verb=Sigma_Algebra=, \verb=Measure_Space=, \verb=Bochner_Integration=, \verb=Caratheodory=, and \verb=Lebesgue_Measure=.
...
Measurability is inherent in probability theory (i.e. events are measurable sets and random variables are measurable functions). The automation works similar to automation of continuity described in Section~\ref{subsection:topology}: using a rule format which allows us to blindly apply introduction rules. A difference is that measurability is also integrated as a plug-in into the simplifier, to solve measurability assumptions. To use the measurability prover, the user needs to annotate all occuring measurability assumption with the \texttt{[measurable]} attribute. This measurability prover then tries to massage all added assumptions into the right form and proves measurability statements by applying them as introduction rules. As a special case it also allows to cut of sub-terms with a countable range, i.e. $f~(g~x)~x$ can be reduced to the measurability of $g$ and $f~c~x$ for all $c$ in the range of $g$. 
...
define the integral to be zero if not measurable.
}
...
Supporting automation can always be improved, and it was at times frustrating that automated tools would get stuck on seemingly trivial matters like determining whether an instance of zero should be interpreted as a real or an non-negative extended real. As we remarked in Section~\ref{subsection:calculus}, ordinary calculations with integrals was often the most painful part of the formalization. It would be especially useful to have better automated support for such calculations, that either implement features of computer algebra systems in a proof-producing framework, or reconstruct formal proofs of such results from suitable certificates.
