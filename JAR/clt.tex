...
\Snippet{tendstoadd} 
Here \isamath{(f \xrightarrow{\hphantom{AAA}} x)~F} is an abbreviation for {\tt filterlim f F (nhds x)}. This avoids the need to formalize endless variations of the same theorem; we only need to instantiate the general version to ehe relevant filters. Details can be found in \cite{hoelzl:et:al:13}.
