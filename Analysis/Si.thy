...
proof -
  ...
  proof -
    have "LBINT x=-\<infinity>..\<infinity>. inverse (1 + x^2) = (LBINT x=-\<infinity>..0. inverse (1 + x^2)) +
      ...
      apply (subst power2_eq_square, auto)
      using Billingsley_ex_17_5 unfolding interval_lebesgue_integral_def einterval_def max_def min_def
      apply auto
...
lemma Billingsley_26_15:
  assumes "T \<ge> 0"
  shows "\<And>\<theta>. LBINT t=0..T. sin (t * \<theta>) / t = sgn \<theta> * Si (T * \<bar>\<theta>\<bar>)"
  sorry
